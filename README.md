...
Once your backend is configured, you can start it by running `npm ci` in the `services/roomService` directory (the first time you run it, you will also need to run `npm install`).
...
In the `frontend` directory, run `npm start` (again, you'll need to run `npm ci` the very first time). After several moments (or minutes, depending on the speed of your machine), a browser will open with the frontend running locally.
