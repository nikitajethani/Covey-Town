...
type CoveyAppUpdate =
  ...
  | { action: 'disconnect' }
  | {action: 'sendPublicMessage'; message: string, time: string}
  | {action: 'sendPrivateMessage'; message: string; toPlayer: string, time: string}
  | {action: 'receivePublicMessage'; message: string, fromPlayer: string, time: string}
  | {action: 'receivePrivateMessage'; message: string; fromPlayer: string, time: string}
  | {action: 'updateSeen'}
  ;
...
function appStateReducer(state: CoveyAppState, update: CoveyAppUpdate): CoveyAppState {
  ...
  switch (update.action) {
    ...
    case 'sendPublicMessage': 
      nextState.messageHistory.messageHistory.push({id: nanoid(), privacy: 'public', from: 'Me', to: 'Everyone', message: update.message, time: update.time});     
      break;
    
    case 'sendPrivateMessage': 
      nextState.messageHistory.messageHistory.push({id: nanoid(), privacy: 'private', from: 'Me', to: update.toPlayer, message: update.message, time: update.time});
      break;

    case 'receivePublicMessage':
      nextState.messageHistory.messageHistory.push({id: nanoid(), privacy: 'public', from: update.fromPlayer, to: 'Everyone', message: update.message, time: update.time});
      nextState.unseenMessages += 1;
      break;
  
    case 'receivePrivateMessage': 
      nextState.messageHistory.messageHistory.push({id: nanoid(), privacy: 'private', from: update.fromPlayer, to: 'Me', message: update.message, time: update.time});
      nextState.unseenMessages += 1;
      break;

    case 'updateSeen': 
      nextState.unseenMessages = 0;
      break;
  }
...
async function GameController(initData: TownJoinResponse,
  ...
  socket.on('publicMessage', (msg: string, timenow: string) => {
    dispatchAppUpdate({ action: 'sendPublicMessage', message: msg, time: timenow });
  });

  socket.on('privateMessage', (msg: string, to: string, timenow: string) => {
    dispatchAppUpdate({ action: 'sendPrivateMessage', message: msg, toPlayer: to, time: timenow});
  });

  socket.on('rPublicMessage', (msg: string, from: string, timenow: string) => {
    dispatchAppUpdate({ action: 'receivePublicMessage', message: msg, fromPlayer: from, time: timenow });
  });

  socket.on('rPrivateMessage', (msg: string, from: string, timenow: string) => {
    dispatchAppUpdate({ action: 'receivePrivateMessage', message: msg, fromPlayer: from, time: timenow});
  });

  socket.on('messagesSeen', () => {
    dispatchAppUpdate({ action: 'updateSeen'});
  });
