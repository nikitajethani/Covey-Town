...
describe('TownServiceApiSocket', () => {
  ...
  it('Informs all players when a public message is sent', async () => {
    const town = await createTownForTesting();
    const joinData = await apiClient.joinTown({coveyTownID: town.coveyTownID, userName: 'nikita'});
    const joinData2 = await apiClient.joinTown({coveyTownID: town.coveyTownID, userName: 'meghan'});
    const joinData3 = await apiClient.joinTown({coveyTownID: town.coveyTownID, userName: 'daisy'});
    const {socket: socket1} = TestUtils.createSocketClient(server, joinData.coveySessionToken, town.coveyTownID);
    const socket2 = TestUtils.createSocketClient(server, joinData2.coveySessionToken, town.coveyTownID).socket;
    const socket3 = TestUtils.createSocketClient(server, joinData3.coveySessionToken, town.coveyTownID).socket;
    socket1.emit('sendPublicMessage', 'hi!', 'nikita');
    socket1.emit('publicMessage', 'hi!', getTime());
    socket2.emit('rPublicMessage', 'hi!', 'nikita', getTime());
    socket3.emit('rPublicMessage', 'hi!', 'nikita', getTime());
  });
  it('Informs player when a private message is sent', async () => {
    const town = await createTownForTesting();
    const joinData = await apiClient.joinTown({coveyTownID: town.coveyTownID, userName: 'nikita'});
    const joinData2 = await apiClient.joinTown({coveyTownID: town.coveyTownID, userName: 'meghan'});
    const {socket: socket1} = TestUtils.createSocketClient(server, joinData.coveySessionToken, town.coveyTownID);
    const socket2 = TestUtils.createSocketClient(server, joinData2.coveySessionToken, town.coveyTownID).socket;
    socket2.emit('sendPrivateMessage', 'hi!', 'nikita', 'daisy');
    socket2.emit('privateMessage', 'hi!', 'nikita', getTime());
    socket1.emit('rPrivateMessage', 'hi!', 'daisy', getTime());
  });
});
