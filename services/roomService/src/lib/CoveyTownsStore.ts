...
export default class CoveyTownsStore {
  ...
  /**
  * Gets the usernames in the town 
  * Returns the list of usernames and true if the town exists or false if it doesn't. 
  * 
  * @param coveyTownID the  id of the town
  */
  getTowninfo(coveyTownID: string): {playernames: string[], response: boolean} {
    const existingTown = this.getControllerForTown(coveyTownID);
    const playernames: string[] = [];
    if (existingTown) {
      existingTown.players.forEach((player) => playernames.push(player.userName));
      return {playernames, response: true};
    }
    return {playernames, response: false};
  }
