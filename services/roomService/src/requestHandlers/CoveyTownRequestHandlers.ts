...
/**
 * Payload sent by client to create get the list of usernames of current player for a Town.
 */
...
/**
 * The format of a response to get a list of usernames of the current players for a Town.  
 */
...
export async function townInfoHandler(requestData: TownInfoRequest): Promise<ResponseEnvelope<TownInfoResponse>> {
  const townsStore = CoveyTownsStore.getInstance();
  const success = townsStore.getTowninfo(requestData.coveyTownID);
  return {
    isOK: success.response,
    response: { currentPlayers: townsStore.getTowninfo(requestData.coveyTownID).playernames },
  };
}
...
/**
 * Gets the current time in HH:MM and appends 'am' or 'pm' to specify when during the day.
 *
 */
function getTime(): string {
  ...
  const timenow = `${hour  }:${  minString}${  part}`;
  return timenow;
}
...
export function townSubscriptionHandler(socket: Socket): void {
  ...
  // Register an event listener for the client socket: if the client sends a public message
  // inform the client socket to display the message on the sender's screen and 
  // inform all the other clients sockets in the town to display the message 
  socket.on('sendPublicMessage', (message: string, from: string) => {
    const timenow = getTime();
    socket.emit('publicMessage', message, timenow);
    socket.broadcast.emit('rPublicMessage', message, from, timenow);
  });

  // Register an event listener for the client socket: if the client sends a private message
  // inform the client socket that to display the message on the sender's screen and 
  // inform the client's socket that receives the message in the town to display the message on the screen
  socket.on('sendPrivateMessage', (message: string, receiver: string, from: string) => {
    const timenow = getTime();
    socket.emit('privateMessage', message, receiver, timenow);
    const receiveplayer = townController.players.find((p) => p.userName === receiver);
    if (receiveplayer) {socket.broadcast.to(receiveplayer.socket).emit('rPrivateMessage', message, from, timenow);}
  });

  // Register an event listener for the client socket: if the client is looking at their messages, 
  // inform the client socket to not display the number of unseen messages
  socket.on('looking at messages', () => {
